...
def findAllGzipedLogs(runDirectory):
    logFiles = glob.glob(runDirectory + '*.log.gz', recursive=False)
    return logFiles
...
for pingIp, pingServerName in cloudServers.items():
    ...
    for log in logs:
        ...
        for pongServerName, data in unidirectionalPingData.items():
            ...
            path = runDirectory + '/pickle/' + pingServerName + '/' + pongServerName
            os.makedirs(path, exist_ok=True)
            picklefile = path + '/' + dateTime  + "_unidirectional.pickle"
